...
class DTADataset(InMemoryDataset):
    ...
    def process(self, xd, target_key, y, smile_graph, target_graph, smiles_vec, proteins_vec):
        ...
        for i in range(data_len):
            ...
            # smile_vec = smiles_vec[smiles]
            # protein_vec = proteins_vec[tar_key]

            GCNData_mol = DATA.Data(x=torch.Tensor(features),
                                    edge_index=torch.LongTensor(edge_index).transpose(1, 0),
                                    y=torch.FloatTensor([labels]))#, smile_vec=smile_vec)
            ...
            GCNData_pro = DATA.Data(x=torch.Tensor(target_features),
                                    ...
                                    y=torch.FloatTensor([labels]))#, protein_vec=protein_vec)
